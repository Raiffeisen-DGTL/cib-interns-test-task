...
@WebServlet(name = "IncomeSocksServlet", value = "/socks/income")
public class IncomeSocksServlet extends HttpServlet {
    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            //выгребаем все носки в список
            List<Socks> allSocks = session.createQuery("from Socks").getResultList();
            if (allSocks.contains(socks)) {
                int id = 0;
                //ищем соответствующие записи и добавляем в них переданное количество носков
