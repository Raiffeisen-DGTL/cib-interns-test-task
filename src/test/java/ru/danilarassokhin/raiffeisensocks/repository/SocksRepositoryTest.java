...
public class SocksRepositoryTest extends EmbeddedTest {
  ...
  @Test
  public void cottonPartOperationEqual() {
    List<Socks> socks = socksRepository.findAll(
        CottonPartEqualityOperation.EQUAL.create((byte) 10)
    );

    Assertions.assertEquals(1, socks.size());
    Assertions.assertEquals("red", socks.get(0).getColor());
  }

  @Test
  public void isColorSpecification() {
    List<Socks> socks = socksRepository.findAll(
        SocksRepository.ofColor("red")
    );
  ...
  @Test
  public void cottonPartCodesCount() {
    Assertions.assertEquals(
        CottonPartEqualityOperation.values().length,
        CottonPartEqualityOperation.codes().size()
    );
  }

  @Test
  public void invalidCottonPartOperation() {
    Assertions.assertNull(CottonPartEqualityOperation.getByCode("invalidCode")
        .orElse(null));
  }
}
